-- Demo-/Beispieldaten für den Quickstart-Stack (NICHT für Produktivbetrieb).
-- Wird vom Quickstart nach dem Backend-Start einmalig eingespielt. Idempotent: fügt nur ein,
-- wenn noch keine Mandanten existieren, damit ein Neustart keine Duplikate erzeugt.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM mandant) THEN
